...

{
	...
	ros::Publisher number_publisher = node_obj.advertise<std_msgs::Int32>("/numbers",10);// The 10 is the buffer size \
                                                                             (number of msgs accumulated before sending)

	//Created a rate object
	ros::Rate loop_rate(10); //sets the frequency in hertz
	...
	while (ros::ok())
	{
		...
		//Created a Int32 message object
		std_msgs::Int32 msg;
